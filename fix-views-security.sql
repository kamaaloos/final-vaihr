-- Fix security for views by recreating them with proper security
...
DROP VIEW IF EXISTS chat_relationship;
...
-- Recreate chat_relationship view with security
CREATE VIEW chat_relationship 
WITH (security_invoker = true) AS
SELECT 
    c.id as chat_id,
    c.driver_id,
...
ALTER VIEW chat_relationship ENABLE ROW LEVEL SECURITY;
