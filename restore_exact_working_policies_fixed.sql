...
-- FIXED: Added proper type casting for UUID/TEXT comparisons
...
-- ==============================================
-- STEP 3: RESTORE EXACT WORKING POLICIES (WITH TYPE CASTING FIXES)
-- ==============================================
...
-- NOTIFICATIONS TABLE POLICIES (FIXED TYPE CASTING)
...
-- SETTINGS TABLE POLICIES (FIXED TYPE CASTING)
CREATE POLICY "Users can view their own settings"
    ON settings FOR SELECT
    TO authenticated
    USING (user_id = auth.uid());

-- TERMS TABLE POLICIES (FIXED TYPE CASTING)
CREATE POLICY "Drivers and admins can view terms"
    ON terms FOR SELECT
    TO authenticated
    USING (EXISTS ( SELECT 1
   FROM users
  WHERE ((users.id = auth.uid()) AND ((users.role = 'driver'::text) OR (users.role = 'admin'::text)))));
