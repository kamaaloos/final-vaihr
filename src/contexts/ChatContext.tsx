...
=======
  ...
  // Debounced presence state update
  // This prevents rapid successive updates that were causing repeating logs
  const debouncedUpdatePresenceState = useCallback(
    debounce((newState: PresenceState) => {
      console.log('Debounced presence state update:', {
        keys: Object.keys(newState),
        hasData: Object.keys(newState).length > 0
      });
...
>>>>>>> ab99f7f7e06f0a04bb444a5ab899627a77d9221c
...
=======
    ...
    presenceChannel.current = supabase.channel(`presence:${currentChat.id}`, {
      config: {
        presence: {
          key: user.id,
        },
      ...
      .on('presence', { event: 'sync' }, () => {
        const newState = presenceChannel.current.presenceState() as PresenceState;
        console.log('Presence sync event:', {
          ...
          values: Object.values(newState)
        });

        // Use debounced update to prevent rapid successive updates
        debouncedUpdatePresenceState(newState);
      })
      .on('presence', { event: 'join' }, ({ key, newPresences }) => {
        ...
        // Update presence state when a user joins
        const currentState = presenceChannel.current.presenceState() as PresenceState;
        console.log('Updating presence state after join:', currentState);
        debouncedUpdatePresenceState(currentState);
      })
      .on('presence', { event: 'leave' }, ({ key, leftPresences }) => {
        ...
        // Update presence state when a user leaves
        const currentState = presenceChannel.current.presenceState() as PresenceState;
        console.log('Updating presence state after leave:', currentState);
        debouncedUpdatePresenceState(currentState);
      })
      .subscribe(async (status) => {
        ...
        if (status === 'SUBSCRIBED') {
          ...
          console.log('Tracking initial presence:', presenceData);
          await presenceChannel.current.track(presenceData);

          // Also update the user's online status in the users table
          await supabase
            .from('users')
            .update({
              online: true,
          ...
          // Notify the other user to join this presence channel
          console.log('Sending presence notification to user:', otherUserId);
          await supabase
          ...
          // Set up a subscription to listen for presence notifications
          const presenceNotificationSubscription = supabase
            .channel(`presence_notifications:${user.id}`)
            .on('postgres_changes', {
              event: 'INSERT',
              schema: 'public',
              ...
              // Join the presence channel mentioned in the notification
              if (payload.new.chat_id === currentChat.id) {
                console.log('Joining presence channel for chat:', currentChat.id);
                await presenceChannel.current.track(presenceData);
              }
            })
            .subscribe();

          // Clean up presence notification subscription
          return () => {
            presenceNotificationSubscription.unsubscribe();
          };
        }
      });
    ...
    // Use debounced update for initial state too
    debouncedUpdatePresenceState({ [user.id]: [initialPresenceData] });
    ...
    return () => {
      ...
      debouncedUpdatePresenceState.cancel();
      // Clean up any pending timeout
      if (presenceUpdateTimeoutRef.current) {
        clearTimeout(presenceUpdateTimeoutRef.current);
      }
    };
  }, [currentChat?.id, user, debouncedUpdatePresenceState]);

  // Memoized presence data for the other user
  const otherUserPresence = useMemo(() => {
    if (!currentChat?.id || !user || !isPresenceReady) {
      console.log('otherUserPresence: Missing required data', {
        hasChatId: !!currentChat?.id,
        hasUser: !!user,
        isPresenceReady,
        presenceState,
        keys: Object.keys(presenceState)
      });
      return null;
    }
    ...
    // Only log if this is a new lookup or if the presence state has actually changed
    const presenceStateKey = JSON.stringify(presenceState);
    const lastStateKey = lastPresenceStateRef.current._key;

    if (presenceStateKey !== lastStateKey) {
      console.log('otherUserPresence: Looking up presence for other user', {
        otherUserId,
        currentUserId: user.id,
        presenceState,
    ...
    const presence = getPresenceByUserId(presenceState, otherUserId);

    if (presenceStateKey !== lastStateKey) {
      console.log('otherUserPresence: Found presence', {
        presence,
        otherUserId,
        presenceState,
        keys: Object.keys(presenceState)
    ...
    // Store the current state key for comparison
    lastPresenceStateRef.current._key = presenceStateKey;

    return presence;
  }, [presenceState, currentChat, user, isPresenceReady]);

  // Memoized online and typing status
  const isOtherUserOnline = useMemo(() => {
    if (!otherUserPresence || !isPresenceReady) return false;
    return otherUserPresence.online ?? false;
  }, [otherUserPresence, isPresenceReady]);

  const isOtherUserTyping = useMemo(() => {
    if (!otherUserPresence || !isPresenceReady) return false;
    return otherUserPresence.isTyping ?? false;
  }, [otherUserPresence, isPresenceReady]);
>>>>>>> ab99f7f7e06f0a04bb444a5ab899627a77d9221c
...
=======
  ...
  }, [debouncedSetTypingStatus, debouncedUpdatePresenceState]);
...
<<<<<<< HEAD
=======
  // Update online and typing status when presence state changes
  useEffect(() => {
    if (!currentChat?.id || !user || !isPresenceReady) {
      return;
    }

    // Check if the presence state has actually changed meaningfully
    const currentStateKey = JSON.stringify(presenceState);
    const lastStateKey = lastPresenceStateRef.current._lastUpdateKey;

    if (currentStateKey === lastStateKey) {
      return; // No meaningful change, skip update
    }

    console.log('Presence state changed, updating status:', {
      previous: lastStateKey ? 'has previous' : 'no previous',
      current: Object.keys(presenceState)
    });
    ...
    // Store current state key for next comparison
    lastPresenceStateRef.current._lastUpdateKey = currentStateKey;
  }, [presenceState, isOtherUserOnline, isOtherUserTyping, currentChat?.id, user, isPresenceReady]);
